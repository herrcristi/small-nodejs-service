...
const AdminsService = require('./admins.service.js');
...
const Public = {
  ...
  getAll: async (req, res, next) => {
    let _ctx = req._ctx;
    _ctx.serviceName = AdminsConstants.ServiceName;
    ...
    try {
      ...
      // get all
      const r = await AdminsService.getAllForReq(req, _ctx);
      if (r.error) {
  ...
  getOne: async (req, res, next) => {
    ...
    try {
      ...
      // get
      const r = await AdminsService.getOne(objID, rf.value.projection, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  post: async (req, res, next) => {
    ...
    try {
      ...
      // post
      const r = await AdminsService.post(req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  delete: async (req, res, next) => {
    ...
    try {
      ...
      // delete
      const r = await AdminsService.delete(objID, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  put: async (req, res, next) => {
    ...
    try {
      ...
      // put
      const r = await AdminsService.put(objID, req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  patch: async (req, res, next) => {
    ...
    try {
      ...
      // patch
      const r = await AdminsService.patch(objID, req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  notification: async (req, res, next) => {
    ...
    try {
      ...
      // post
      const r = await AdminsService.notification(req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
