...
const Public = {
  ...
  collection: async (_ctx) => {
    if (!_ctx.tenantID) {
      console.log(`\nNo tenant for getting admins collection. Stack: ${new Error().stack}`);
      return null;
    }
    // admins collections are per tenant
    let collName = AdminsConstants.ServiceName + `_${_ctx.tenantID}`;
