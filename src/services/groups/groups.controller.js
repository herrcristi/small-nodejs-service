...
const GroupsService = require('./groups.service.js');
...
const Public = {
  ...
  getAll: async (req, res, next) => {
    let _ctx = req._ctx;
    _ctx.serviceName = GroupsConstants.ServiceName;
    ...
    try {
      ...
      // get all
      const r = await GroupsService.getAllForReq(req, _ctx);
      if (r.error) {
  ...
  post: async (req, res, next) => {
    ...
    try {
      ...
      // post
      const r = await GroupsService.post(req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  delete: async (req, res, next) => {
    ...
    try {
      ...
      // delete
      const r = await GroupsService.delete(objID, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  put: async (req, res, next) => {
    ...
    try {
      ...
      // put
      const r = await GroupsService.put(objID, req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  patch: async (req, res, next) => {
    ...
    try {
      ...
      // patch
      const r = await GroupsService.patch(objID, req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
  ...
  notification: async (req, res, next) => {
    ...
    try {
      ...
      // post
      const r = await GroupsService.notification(req.body, _ctx);
      if (r.error) {
        return res.status(r.status).json(await RestMessagesUtils.statusError(r.status, r.error, _ctx));
      }
