...
describe('Base Controller', function () {
  ...
  it('should patch with success', async () => {
    ...
    const service = {
      patch: sinon.stub().callsFake(() => {
        console.log(`\nService.patch called\n`);
        return {
          status: 200,
          value: testObjects[0],
        };
    ...
    // call
    await BaseControllerUtils.patch(service, serviceName, req, res, next);

    // check
    chai.expect(status).to.equal(200);
    chai.expect(service.patch.callCount).to.equal(1);
