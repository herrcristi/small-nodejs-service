...
describe('Base Service', function () {
  ...
  /**
   * put with success with no events and no notifications
   */
  it('should call put with success no events and no notifications', async () => {
    const objInfo = {
      name: 'name',
      field: 'idf1',
    };
  ...
  it('should call put with failed db', async () => {
    ...
    // call
    let res = await BaseServiceUtils.put({ ...config, translate: null }, 'id1', objInfo, _ctx);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
