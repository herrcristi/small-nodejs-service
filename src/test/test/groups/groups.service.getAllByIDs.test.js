...
describe('Groups Service', function () {
  ...
  it('should getAllByIDs with success', async () => {
    ...
    const testGroupsIDs = testGroups.map((item) => item.id);
    ...
    // call
    let res = await GroupsService.getAllByIDs(testGroupsIDs, { id: 1 }, _ctx);
  ...
  it('should getAllByIDs failed tenant', async () => {
    // call
    let res = await GroupsService.getAllByIDs([], {}, { ..._ctx, tenantID: undefined });
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
