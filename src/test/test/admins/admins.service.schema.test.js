...
describe('Admins Service', function () {
  ...
  it('should validate post schema with success', async () => {
    // call
    let res = AdminsService.Validators.Post.validate(postReq);
  ...
  it('should validate post schema for id', async () => {
    ...
    // id is number
    postReq.id = 1;
    res = AdminsService.Validators.Post.validate(postReq);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
  ...
  it('should validate put schema', async () => {
    // nothing is required
    let putReq = {};
    let res = AdminsService.Validators.Put.validate(putReq);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
    ...
    res = AdminsService.Validators.Put.validate(putReq);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
    chai.expect(res.error.details[0].message).to.include('"extra" is not allowed');
  }).timeout(10000);
  ...
  it('should validate patch schema set', async () => {
    // nothing is required
    let patchReq = {};
    let res = AdminsService.Validators.Patch.validate(patchReq);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
    chai.expect(res.error).to.not.exist;
    ...
    res = AdminsService.Validators.Patch.validate(patchReq);
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
    chai.expect(res.error.details[0].message).to.include('"extra" is not allowed');
