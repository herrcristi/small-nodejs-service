...
describe('Classes Service', function () {
  ...
  it('should getAllByIDs with success', async () => {
    ...
    const testClassesIDs = testClasses.map((item) => item.id);
    ...
    // call
    let res = await ClassesService.getAllByIDs(testClassesIDs, { id: 1 }, _ctx);
  ...
  it('should getAllByIDs failed tenant', async () => {
    // call
    let res = await ClassesService.getAllByIDs([], {}, { ..._ctx, tenantID: undefined });
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
