...
describe('Professors Service', function () {
  ...
  it('should getAllByIDs with success', async () => {
    ...
    const testProfessorsIDs = testProfessors.map((item) => item.id);

    // stub
    let stubBase = sinon.stub(DbOpsUtils, 'getAllByIDs').callsFake(() => {
      console.log(`\nDbOpsUtils.getAllByIDs called\n`);
      return { status: 200, value: [...testProfessors] };
    });

    // call
    let res = await ProfessorsService.getAllByIDs(testProfessorsIDs, { id: 1 }, _ctx);
  ...
  it('should getAllByIDs failed tenant', async () => {
    // call
    let res = await ProfessorsService.getAllByIDs([], {}, { ..._ctx, tenantID: undefined });
    console.log(`\nTest returned: ${JSON.stringify(res, null, 2)}\n`);
