<template>
  ...
  <!-- list using v-chip -->
  <div class="pa-1" v-if="type == 'v-chip' && read && readAdmins">
  ...
  <!-- list using v-card -->
  <v-container fluid v-if="type == 'v-card' && read && readAdmins">
    <v-card-title class="d-flex justify-space-between">
      <div>{{ t(title) }}</div>
    </v-card-title>
...
const props = defineProps({
  ...
  title: { type: String, default: 'Admins' },
...
const readAdmins = app?.rolesPermissions?.admins?.read;
const writeAdmins = app?.rolesPermissions?.admins?.write;
