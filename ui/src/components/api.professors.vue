...
async function fetchAll({ page = 1, itemsPerPage = 50, sortBy = [] } = {}) {
  ...
  try {
    ...
    snackbarText.value = t('professors.fetch.error') || 'Error fetching professors';
...
async function add() {
  try {
    ...
    snackbarText.value = t('professors.add.success') || 'Professor added';
    ...
    snackbarText.value = e?.response?.data?.message || t('professors.add.error') || 'Error adding professor';
    snackbarColor.value = 'error';
    snackbar.value = true;
    return false;
  }
...
async function update() {
  try {
    ...
    await Api.updateProfessor(editingItemID.value, itemData);

    snackbarText.value = t('professors.update.success') || 'Professor updated';
    snackbarColor.value = 'success';
    snackbar.value = true;
    ...
    snackbarText.value = e?.response?.data?.message || t('professors.update.error') || 'Error updating professor';
    snackbarColor.value = 'error';
    snackbar.value = true;
...
async function del(itemID) {
  try {
    ...
    snackbarText.value = t('professors.delete.success') || 'Professor deleted';
    snackbarColor.value = 'success';
    snackbar.value = true;
    ...
    snackbarText.value = e?.response?.data?.message || t('professors.delete.error') || 'Error deleting professor';
    snackbarColor.value = 'error';
    snackbar.value = true;
  }
}
